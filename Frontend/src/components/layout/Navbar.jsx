...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { path: '/manage_team', label: 'QUẢN LÝ ĐỘI', icon: ShieldCheck },
  ...
  return (
    <header className={`fixed top-0 w-full z-[100] transition-all duration-300 flex justify-between items-center px-6 md:px-12 py-4 border-b-2 border-outline-variant ${scrolled ? 'bg-background/80 backdrop-blur-md shadow-lg shadow-black/50' : 'bg-background/95 backdrop-blur-sm'}`}>
      ...
      <div className="flex items-center gap-4">
        {currentUser ? (
          <div className="relative" ref={dropdownRef}>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-surface-charcoal border border-outline-variant shadow-lg z-[110] py-2 clip-corner animate-fadeIn">
                ...
                {currentUser.globalRole === 'ADMIN' && (
                  <button
                    onClick={() => { navigate('/admin_dashboard'); setUserMenuOpen(false); }}
          ...
          <>
            <NavLink
              to="/login"
              className={({ isActive }) => `font-display text-sm uppercase tracking-wider hidden sm:inline-block pb-1 transition-colors ${
                isActive ? 'text-primary-red border-b-2 border-primary-red' : 'text-off-white/70 hover:text-off-white'
              }`}
            >
              LOGIN
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-surface-charcoal border-b-2 border-outline-variant md:hidden z-50 animate-slideUp">
            ...
            {navItems.map(({ path, label, icon: Icon }) => (
              <NavLink
                ...
                className={({ isActive }) => `font-display text-sm uppercase tracking-wider py-3 px-4 flex items-center gap-3 transition-colors ${
                  isActive ? 'text-primary-red bg-primary-red/10' : 'text-off-white/70 hover:text-off-white'
                }`}
              >
                <Icon size={16} /> {label}
              </NavLink>
              ...
              {!currentUser ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="w-full text-left font-display text-sm uppercase tracking-wider py-3 px-4 text-off-white/70 hover:text-off-white flex items-center gap-2"
                  >
                  ...
                  {currentUser.globalRole === 'ADMIN' && (
                    <button
                      ...
                      className="w-full text-left font-display text-sm uppercase tracking-wider py-3 px-4 text-warning-amber hover:text-warning-amber/80 flex items-center gap-2"
                    >
                  ...
                  <button
                    ...
                    className="w-full text-left font-display text-sm uppercase tracking-wider py-3 px-4 text-primary-red hover:text-off-white flex items-center gap-2"
                  >
