...
const Lobby = () => {
  ...
  const handleAdminSaveScore = async (gameId) => {
    ...
    try {
      const activeGame = seriesData.games.find(g => g.id === gameId);
      const payload = {
        ...
        teamAPicks: activeGame ? activeGame.teamAPicks : [],
        teamABans: activeGame ? activeGame.teamABans : [],
        teamBPicks: activeGame ? activeGame.teamBPicks : [],
        teamBBans: activeGame ? activeGame.teamBBans : []
      };
    ...
    <div className="min-h-screen bg-[#0f1923] text-white p-4 flex flex-col relative pb-32 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center mt-16">
        <h2 className="text-2xl font-bold mb-8 uppercase tracking-widest text-gray-300">
          VÁN {activeGameSafe.gameNumber} {!isMapVeto && activeGameSafe.map !== 'CHƯA CHỌN' ? `- BẢN ĐỒ: ${activeGameSafe.map}` : ''} - {isMapVeto ? (currentMapAction?.action === 'PICK_SIDE' ? 'ĐANG CHỜ CHỌN PHE' : (currentMapAction?.action === 'BAN' ? 'ĐANG CHỜ CẤM MAP' : 'ĐANG CHỜ CHỌN MAP')) : (isAgentDraftComplete ? 'ĐÃ HOÀN TẤT' : currentAgentAction?.action === 'BAN' ? 'ĐANG CHỜ CẤM TƯỚNG' : 'ĐANG CHỜ CHỌN TƯỚNG')}
        </h2>
