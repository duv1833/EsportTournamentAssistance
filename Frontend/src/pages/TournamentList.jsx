...
export default function TournamentList() {
  ...
  const handleCreateTournament = async (e) => {
    ...
    if (parseInt(createForm.maxTeams) <= 0) {
      setTournamentError('Số lượng đội tối đa phải lớn hơn 0!');
      return;
    }

    if (createForm.startDate && createForm.endDate) {
      if (new Date(createForm.endDate) < new Date(createForm.startDate)) {
        setTournamentError('Ngày kết thúc không được nhỏ hơn ngày bắt đầu!');
        return;
      }
    }
    if (!currentUser) {
